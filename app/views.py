        ...
        if form.is_valid():
            ...
            if (re.fullmatch(regexteacher, email)):
                ...
                messages.success(request,
                                 'Congratulations!! You are registered successfully as a Teacher, Your Username is Your Email')
            elif (re.fullmatch(regexstudent, email)):
                user.username = email[:11]
                user.is_active = False
                user.save()
                messages.success(request,
                                 'Congratulations!! You are registered successfully as a Student , Your Username is your roll Number')
            else:
...
def ImportExport(request):
    ...
    if (request.method == 'POST'):
        ...
        return render(request, 'attendence.html',
                      {'classname': classname, 'subjectname': subjectname, 'date': date, 'df': finallist})
...
def DetailedStudentDashboard(request, subID):
    attendence = Attendence.objects.filter(rollno=int(request.user.username)).filter(subjectname_id=subID)
    if request.method == 'POST':
        startdate = request.POST.get('startdate')
        enddate = request.POST.get('enddate')

        attendence = Attendence.objects.filter(rollno=int(request.user.username)).filter(
            date__range=[startdate, enddate]).filter(subjectname_id=subID)

    return render(request, 'StudentDashboard.html', {'subID': subID, 'attendence': attendence})
...
def SubjectFilter(request):
    ...
    attendence = Attendence.objects.filter(rollno=int(request.user.username)).filter(subjectname=subject_name).order_by(
        'date')
...
def ExportExcel(request):
    ...
    if request.method == 'POST':
        ...
        if startdate and enddate:
            attendence = Attendence.objects.filter(classname=class_name).filter(subjectname=subject_name).filter(
        ...
        if startdate and enddate:
            totalattcount = ClassAttendence.objects.filter(classname=class_name).filter(
                subjectname=subject_name).filter(date__range=[startdate, enddate]).count()
        else:
            totalattcount = ClassAttendence.objects.filter(classname=class_name).filter(
                subjectname=subject_name).count()
